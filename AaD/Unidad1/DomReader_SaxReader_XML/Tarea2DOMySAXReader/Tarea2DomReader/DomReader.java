...
public class DomReader {
    public static void main(String[] args) throws Exception {
        ...
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance(); // 1 FABRICA
        DocumentBuilder builder = factory.newDocumentBuilder(); // 2 CONSTRUCTOR
        Document xmlDoc = (Document) builder.parse(xmlFile); // 3 ARBOL DOM CREADO


        Element rootElement = xmlDoc.getDocumentElement(); // 4 REPRESENTA UNA ETIQUETA NODO, PERMITE ACCEDER A SUS ATRIBUTOS Y CONTENIDO (LIBRARY)


        NodeList nodosHijo = rootElement.getChildNodes();// 5 OBTENER TODOS LOS NODOS HIJOS DEL ELEMENTO RAIZ CON NODELIST (BOOKS)
        ArrayList<Book> books = new ArrayList<>();// 6 CREAMOS ARRAYLIST DE "BOOKS".

        // 7 Recorremos todos los nodos hijo del elemento Raiz con un FOR,
        for (int index = 0; index < nodosHijo.getLength(); index++) {
            Node currentNode = nodosHijo.item(index);

            // 8 Con este IF nos aseguramos que el nodo actual es un ELEMENTO.
            if (currentNode.getNodeType() == Node.ELEMENT_NODE) {


                Element bookElement = (Element) currentNode; // 8 SI EL NODO ES UN ELEMENTO, LO CONVERETIMOS A ELEMENT PARA ACCEDER A SUS ELEMENTOS

                // 9 OBTENEMOS LOS DATOS DE CADA LIBRO:
                ...
                // 10 OBTENEMOS PRECIO Y ATRIBUTO:
                ...
                // 11 OBTENEMOS TODOS LOS AUTORES QUE TENGA EL LIBRO Y SU ATRIBUTO.
                NodeList authorsList = bookElement.getElementsByTagName("author");

                StringBuilder authorsBuilder = new StringBuilder(); // SB PARA CONSTRUIR INFO SOBRE AUTORES EN CADA ITERACION
